...
8. **Verify Claims:** Determine the project's actual lint/type-check/test commands from its own config (`pyproject.toml`, `tox.ini`, CI workflows, `README`) rather than assuming a toolchain. Run them rather than guessing, and record the exact commands and their outcomes in the report. If no such tooling is configured, say so explicitly as a finding.
...
- **Type Annotations:** Ensure completeness and idiomatic syntax for the project's declared Python floor (e.g. `X | None`, builtin generics, `Self` on 3.10+). Avoid unparameterized containers or bare `Any`.
...
- **Project Test Contract:** Derive marker/strictness/async rules from the project's own config (`pyproject.toml`, `pytest.ini`, `tox.ini`) rather than assuming a specific contract. If no test suite exists, report that as a finding rather than skipping the section.
